...
description: Import skills, memory files, and API keys from an existing OpenClaw or Hermes installation.
---

# Migrating from OpenClaw or Hermes

*Bring your skills, memory files, and keys across — after a dry run.*
...
- NeoAgent installed with the [repository installer](getting-started.md)
...
:::caution Imported Markdown is source material, not verified memory
...
**"No OpenClaw or Hermes installation detected"** — Installation must be at the
default path. If it's elsewhere, migrate manually:
...
**"Permission denied" errors** — Check read permissions on source directories
and write permissions on `~/.neoagent/`.

**Migration partially completed** — Safe to re-run. Only new files are copied;
