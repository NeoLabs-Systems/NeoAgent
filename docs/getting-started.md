...
description: Install NeoAgent, pick a setup mode, create the first account, and verify the service.
---

# Installation

*From a bare machine to a running, signed-in NeoAgent service.*
...
Run the repository installer. It clones NeoAgent directly from GitHub and
requires Git, Node.js 20+, and npm for dependencies:
...
2. Configure a hosted provider or local Ollama under **Admin › Providers** (the
   first account is the admin), with `neoagent env`, or during `neoagent setup`.
...
:::note Optional capabilities
Docker, browser isolation, voice, integrations, and media extraction are
optional. Their absence does not block core setup or sign-in.
:::
