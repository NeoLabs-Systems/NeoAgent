...
class DurableRunRuntime {
  ...
  async run(userId, userMessage, options = {}, modelOverride = null) {
    ...
    try {
      ...
      // matcher because selecting them again used to crowd out the real tool and
      // force two extra discovery turns.
      ...
      while (true) {
        ...
        if (decision.kind === DECISION_KINDS.ACT) {
          ...
          const executeOne = async ({ call, definition, isReadOnly }) => {
            ...
            if ((call.name === 'search_tools' || call.name === 'activate_tools') && success) {
