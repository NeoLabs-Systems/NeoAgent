...
function latestTag(run, sourceDirectory, pattern) {
...
function resolveTargetBranch(run, sourceDirectory, releaseChannel) {
  ...
  const preferred = choosePreferredBranchForChannel(channel, {
    stable: latestTag(run, sourceDirectory, 'v[0-9]*.[0-9]*.[0-9]*'),
...
function checkoutReleaseBranch(run, sourceDirectory, targetBranch) {
  ...
  if (currentBranch !== targetBranch) {
    ...
    if (dirty) {
      throw new Error(
        `Cannot switch the source installation to ${targetBranch} while it has local changes. `
        + `Commit or stash the changes in ${sourceDirectory}, then retry.`,
      );
    }
    ...
    const localBranch = run('git', [
      'show-ref',
      '--verify',
      '--quiet',
      `refs/heads/${targetBranch}`,
    ], { cwd: sourceDirectory });
    const checkoutArgs = localBranch.status === 0
    ...
    runChecked(
      ...
      checkoutArgs,
      { cwd: sourceDirectory },
      `Could not check out ${targetBranch}`,
    );
  }
  runChecked(
    run,
    'git',
    ['pull', '--rebase', '--autostash', 'origin', targetBranch],
    { cwd: sourceDirectory },
    `Could not update origin/${targetBranch}`,
  );
}
...
function prepareSourceInstallation({
  ...
  if (!createdCheckout && gitOutput(run, sourceDirectory, ['status', '--porcelain'])) {
    throw new Error(
      `${sourceDirectory} has local changes. Commit or stash them, or set `
      + 'NEOAGENT_SOURCE_DIR to an empty installation directory, then retry.',
    );
  }
  ...
  checkoutReleaseBranch(run, sourceDirectory, targetBranch);
