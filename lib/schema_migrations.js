...
const { SKILL_VERSIONING_TABLES } = require('./shared_schema');
const { isManagedDeployment } = require('../server/utils/deployment');
...
// Every agent used to be seeded with OpenAI speech-to-text, so users without an
// OpenAI key got failing voice notes. Rows still holding that seeded pair move
// to 'auto', which keeps OpenAI when its key exists and otherwise uses Gemini
// or Deepgram. An explicit OpenAI choice now stores 'gpt-transcribe', so this
// never matches again.
function migrateSeededSttDefaultsToAuto(db) {
  const seededProvider = "value IN ('\"openai\"', 'openai')";
  const seededModel = "value IN ('\"gpt-live-transcribe\"', 'gpt-live-transcribe')";
  db.transaction(() => {
    db.prepare(`
      UPDATE agent_settings SET value = '"auto"'
      WHERE key = 'voice_stt_provider' AND ${seededProvider}
        AND EXISTS (
          SELECT 1 FROM agent_settings AS model
          WHERE model.user_id = agent_settings.user_id
            AND model.agent_id = agent_settings.agent_id
            AND model.key = 'voice_stt_model' AND model.${seededModel}
        )
    `).run();
    db.prepare(`
      UPDATE user_settings SET value = '"auto"'
      WHERE key = 'voice_stt_provider' AND ${seededProvider}
        AND EXISTS (
          SELECT 1 FROM user_settings AS model
          WHERE model.user_id = user_settings.user_id
            AND model.key = 'voice_stt_model' AND model.${seededModel}
        )
    `).run();
    db.prepare(`UPDATE agent_settings SET value = '""' WHERE key = 'voice_stt_model' AND ${seededModel}`).run();
    db.prepare(`UPDATE user_settings SET value = '""' WHERE key = 'voice_stt_model' AND ${seededModel}`).run();
  })();
}

// Live voice speaks through speech-to-speech models only; the composed
// STT → LLM → TTS pipeline and its settings are gone.
function removeRetiredVoicePipelineSettings(db) {
  const retiredKeys = ['voice_tts_provider', 'voice_tts_model', 'voice_tts_voice', 'voice_media_mode'];
  const placeholders = retiredKeys.map(() => '?').join(', ');
  db.prepare(`DELETE FROM user_settings WHERE key IN (${placeholders})`).run(...retiredKeys);
  db.prepare(`DELETE FROM agent_settings WHERE key IN (${placeholders})`).run(...retiredKeys);

  // An earlier live voice implementation stored realtime models and voices
  // under the same voice_live_* keys, marked by voice_runtime_mode. Those values
  // mean nothing to the live models now and would pin the account to them over
  // the server default, so scopes carrying the marker start from the defaults.
  const liveKeys = "('voice_live_provider', 'voice_live_model', 'voice_live_voice', 'voice_runtime_mode')";
  db.transaction(() => {
    db.prepare(`
      DELETE FROM agent_settings
      WHERE key IN ${liveKeys}
        AND EXISTS (
          SELECT 1 FROM agent_settings AS marker
          WHERE marker.user_id = agent_settings.user_id
            AND marker.agent_id = agent_settings.agent_id
            AND marker.key = 'voice_runtime_mode'
        )
    `).run();
    db.prepare(`
      ...
      WHERE key IN ${liveKeys}
        AND EXISTS (
          SELECT 1 FROM user_settings AS marker
          WHERE marker.user_id = user_settings.user_id
            AND marker.key = 'voice_runtime_mode'
        )
    `).run();
  })();
...
function migrateAiModelHealth(db) {
  db.exec(`
    CREATE TABLE IF NOT EXISTS ai_model_health (
      user_id INTEGER NOT NULL,
      agent_id TEXT NOT NULL,
      provider_id TEXT NOT NULL,
      model_selection_id TEXT NOT NULL,
      failure_scope TEXT NOT NULL
        CHECK(failure_scope IN ('model', 'provider')),
      ...
      created_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT DEFAULT (datetime('now')),
      PRIMARY KEY (user_id, agent_id, provider_id, model_selection_id),
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    );

    CREATE INDEX IF NOT EXISTS idx_ai_model_health_active
      ON ai_model_health(user_id, agent_id, cooldown_until_ms);
  `);
}

// Per-account admin plus the delegation chain (who manages whom, the invite
// links that set it up, and an audit trail of every change). Replaces the old
// env-credential admin login and its singleton 2FA tables.
function migrateAccessControl(db) {
  const userColumns = new Set(
    db.prepare('PRAGMA table_info(users)').all().map((column) => column.name),
  );
  if (!userColumns.has('is_admin')) {
    db.exec('ALTER TABLE users ADD COLUMN is_admin INTEGER NOT NULL DEFAULT 0');
    // Installs from before per-account admin: a self-hosted install's only
    // account is its operator -- unless it is still a pending email signup, or
    // the deployment is managed (that account is a tenant, not the operator).
    // Everyone else gets no automatic admin; startup and `neoagent admin`
    // point at `neoagent admin grant <username>`.
    const accounts = db.prepare(
      "SELECT id, email, email_verified_at FROM users LIMIT 2",
    ).all();
    const [only] = accounts;
    const pendingSignup = only
      && only.email_verified_at == null
      && String(only.email || '').trim() !== '';
    if (accounts.length === 1 && !pendingSignup && !isManagedDeployment()) {
      db.prepare('UPDATE users SET is_admin = 1 WHERE id = ?').run(only.id);
    }
  }

  db.exec(`
    CREATE TRIGGER IF NOT EXISTS users_first_account_is_admin
    ...
    CREATE INDEX IF NOT EXISTS idx_delegation_invites_issuer
      ON delegation_invites(issuer_user_id, created_at DESC);

    CREATE TABLE IF NOT EXISTS user_delegations (
      managed_user_id INTEGER PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
      manager_user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      allowed_permissions_json TEXT NOT NULL DEFAULT '[]',
      invite_id TEXT REFERENCES delegation_invites(id) ON DELETE SET NULL,
      created_at TEXT NOT NULL DEFAULT (datetime('now')),
      CHECK (managed_user_id <> manager_user_id)
    );

    CREATE INDEX IF NOT EXISTS idx_user_delegations_manager
      ON user_delegations(manager_user_id);
