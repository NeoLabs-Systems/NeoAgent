...
function resolvePreferredGitBranch(channel) {
  ...
  const stableVersion = latestGitTagVersion('v[0-9]*.[0-9]*.[0-9]*', { excludePrerelease: true });
...
function ensureGitBranchForReleaseChannel(targetBranch) {
  return replaceSourceCheckoutWithRemote({
    run: (command, args, options = {}) => runQuiet(command, args, options),
...
function backupRuntimeData({ prefix = 'pre-update' } = {}) {
  ...
  if (fs.existsSync(AGENT_DATA_DIR)) {
    fs.cpSync(AGENT_DATA_DIR, path.join(target, 'agent-data'), {
      recursive: true,
      force: false,
      errorOnExist: false,
    });
...
async function cmdApiKeySetup() {
  ...
  logInfo('Choose one or more hosted providers. You can also configure these later from the Admin tab in the app or with `neoagent env set`.');
  ...
  if (choices.length === 0) {
    logInfo('Skipping hosted provider keys. Add them from the Admin tab in the app or with `neoagent env set`. Local Ollama remains available if OLLAMA_URL is reachable.');
    rememberInstallAction('Add a hosted provider later from the Admin tab in the app or with `neoagent env set`.');
    return 0;
  }
...
function writeDefaultEnvFile({ remindProviderSetup = true, port = null } = {}) {
  ...
  if (remindProviderSetup) {
    rememberInstallAction('Add provider keys with `neoagent setup`, `neoagent env set KEY VALUE`, the Admin tab in the app, or the login commands when you are ready.');
  }
}
  ...
  if (normalizedProfile === 'quick') {
    ...
    if (!options.nonInteractive && process.stdin.isTTY && process.stdout.isTTY) {
      ...
      if (!['n', 'no'].includes(configureProvider.trim().toLowerCase())) {
        ...
        rememberInstallAction('Connect an AI provider later from the Admin tab in the app or with `neoagent env set`.');
      }
    } else {
      rememberInstallAction('Connect an AI provider later from the Admin tab in the app or with `neoagent env set`.');
    }
    ...
    rememberInstallAction(
      'Configure AI providers from the Admin tab in the app or `neoagent env`. Continue integrations, voice, and optional tools in the NeoAgent app.',
    );
    completedSections = ['core'];
  } else {
...
async function fetchLocalHandshake(port) {
  try {
...
async function ensureComputerRuntime({ required = false, downloadImage = true } = {}) {
  ...
  if (getTerminalEnv() === TERMINAL_ENV_HOST) {
    // The runtime is this machine. Nothing to install, nothing to download.
    logOk('Host computer runtime is ready (agents run on this machine, unisolated)');
    return true;
  }
  if (getTerminalEnv() === TERMINAL_ENV_DOCKER) {
    return ensureDockerComputerRuntime({ required, downloadImage });
  }
  ...
  try {
    ...
    const { restoreQemuComputerRuntime } = require('./qemu_runtime_install');
    const qemuAvailable = Boolean(resolveQemuSystemBinary() && resolveQemuImgBinary());
    if (!qemuAvailable) logInfo('QEMU computer runtime is missing. Restoring…');
    const restored = restoreQemuComputerRuntime({
      platform: detectPlatform(),
      commandExists,
      runOrThrow,
      logInfo,
      allowHostInstall: !qemuAvailable,
    });
    if (restored.action === 'signed') {
      logOk('Enabled hardware acceleration on the bundled QEMU runtime');
    } else if (restored.action === 'copied') {
      logOk('Restored the signed QEMU runtime from the installed package');
    } else if (restored.action === 'staged') {
      logOk('Installed and staged the QEMU computer runtime');
    }
    ...
    logWarn(message);
    rememberInstallAction('Install QEMU, then run `neoagent repair` to stage the computer runtime and guest image.');
    return false;
  }
}

// TERMINAL_ENV=docker: the runtime is the Docker daemon plus the guest-agent
// image, which is content addressed over the guest payload and rebuilds itself
// whenever that payload changes.
function computerRuntimeDetail(backend, readiness) {
  if (readiness.isolated === false) return `${backend} · runs on this machine, unisolated`;
  const guestImage = readiness.imageReady ? 'guest image ready' : 'guest image is prepared on first start';
  return `${readiness.accelerator || backend} · ${guestImage}`;
}

async function ensureDockerComputerRuntime({ required = false, downloadImage = true } = {}) {
  try {
    const { createComputerVmManager } = require('../server/services/runtime/vm_manager');
    const readiness = await createComputerVmManager().prepareRuntime({ downloadImage });
    logOk(`Docker computer runtime is ready (${readiness.image || 'guest image builds on first start'})`);
    return true;
  } catch (err) {
    const message = `Computer runtime preparation did not complete: ${err.message}`;
    if (required) throw new Error(message);
    logWarn(message);
    rememberInstallAction('Start Docker, then run `neoagent repair` to build the guest container image.');
    return false;
  }
}
...
async function cmdInstall({ args = [], showBanner = true } = {}) {
  if (args.includes('--adopt-existing')) {
    await cmdAdoptExistingInstallation({ showBanner });
    return;
  }
  const options = parseSetupArguments(args);
  if (options.releaseChannel !== null) {
    const requestedChannel = parseReleaseChannel(options.releaseChannel);
    if (!requestedChannel) {
      throw new Error('Usage: neoagent install --channel stable|beta');
    }
    writeReleaseChannelToEnvFile(requestedChannel, ENV_FILE);
  ...
  try {
    ...
    if (!cliJsonOutput) {
      ...
      logInfo('The first account you create becomes this server’s admin. Manage admins with `neoagent admin`.');
    }
  } catch (error) {
    writeSetupState(SETUP_STATE_FILE, {
      profile,
...
async function cmdAdoptExistingInstallation({ showBanner = true } = {}) {
  if (showBanner) {
    cliBanner(`Adopt existing ${APP_NAME} data`, 'source installation migration');
  }
  heading('Validate source installation');
  const hasExistingRuntimeData = fs.existsSync(ENV_FILE)
    || fs.existsSync(DATABASE_FILE)
    || (fs.existsSync(AGENT_DATA_DIR) && fs.readdirSync(AGENT_DATA_DIR).length > 0);
  if (!hasExistingRuntimeData) {
    throw new Error('No existing NeoAgent runtime data was found. Run `neoagent install` instead.');
  }
  ensureRuntimeDirs();
  if (!dependenciesReady()) {
    throw new Error(`The source installation at ${APP_DIR} is missing runtime dependencies.`);
  }
  if (!hasBundledWebClient(WEB_CLIENT_DIR)) {
    throw new Error(`The source installation at ${APP_DIR} is missing the bundled web client.`);
  }
  logOk(`Source checkout ready at ${APP_DIR}`);

  heading('Backup');
  const backup = backupRuntimeData({ prefix: 'pre-source-migration' });
  logOk(`Configuration, database, and agent data backed up to ${backup}`);

  heading('Switch service installation');
  cmdStop();
  ...
  if (listNeoAgentServerProcesses().length > 0) {
    throw new Error('NeoAgent did not stop in time. The backup is intact and the service was not replaced.');
  }
  ...
  const port = loadEnvPort();
  await waitForServerReady(port);
  logOk(`Existing runtime data is now served by ${APP_DIR}`);
  logOk(`NeoAgent is running on http://localhost:${port}`);
}
...
async function cmdStart() {
  ...
  // Spawning the service only means the process was launched. Callers (the
  // desktop app included) treat a successful start as "the backend answers
  // now", so wait for the readiness handshake before reporting success.
  await waitForServerReady(loadEnvPort());
  logOk(`${APP_NAME} is reachable`);
}
...
function managesLocalService() {
  const platform = detectPlatform();
  if (platform === 'macos' && fs.existsSync(PLIST_DST)) return true;
  if (platform === 'linux' && fs.existsSync(SYSTEMD_UNIT)) return true;
  try {
...
// Installing over a running NeoAgent starts a second server on the same port,
// so the old one has to let go of it first. The port is the portable signal:
// `ps` is unavailable on Windows, where the update crashed with EADDRINUSE.
async function releasePortForSetup(port) {
  if (!(await isPortOpen(port))) {
    return;
  }
  // Only ever stop a server this installation owns. Anything else on the port
  // belongs to another program and is not ours to kill.
  if (!(await isOwnInstanceReachable(port)) && !managesLocalService()) {
    const error = new Error(
      `Port ${port} is used by another program. Free the port, or set PORT in ${ENV_FILE}, then try again.`,
    );
    error.code = 'SETUP_PORT_IN_USE';
    throw error;
  }
  logInfo(`Stopping the NeoAgent instance already listening on port ${port}`);
  cmdStop();
  ...
  const error = new Error(
    `Port ${port} is still in use. Stop whatever is listening on it, then run the update again.`,
  );
  error.code = 'SETUP_PORT_IN_USE';
  throw error;
}
...
async function cmdStatus({ showBanner = true } = {}) {
  ...
  const platform = detectPlatform();
  const installedVersion = currentInstalledVersionLabel();
  const runningVersion = running
    ? (await fetchLocalHandshake(port))?.serverVersion || null
    : null;
  ...
  try {
    ...
    const { getTerminalEnv } = require('../server/utils/deployment');
    const backend = getTerminalEnv();
    const computer = createComputerBackend().vmManager.getReadiness();
    statusLine(
      computer.ready,
      'computer',
      computer.ready
        ? computerRuntimeDetail(backend, computer)
  ...
  // Reading admins opens the database, so only do it once one exists.
  if (fs.existsSync(DATABASE_FILE)) {
    const { listAdmins, needsAdminGrant } = require('../server/services/access/admin');
    const admins = listAdmins();
    const missing = needsAdminGrant();
    statusLine(
      !missing,
      'admin',
      admins.length > 0
        ? admins.map((admin) => admin.username).join(', ')
        : (missing ? 'no account is an admin' : 'first account created becomes admin'),
      missing ? 'run: neoagent admin grant <username>' : '',
    );
  }
  ...
  if (!cliJsonOutput) {
    ...
    if (runningVersion) console.log(`  serving   ${runningVersion}`);
    console.log(`  channel   ${releaseChannelSummary(releaseChannel)}`);
  }
  if (staleProcess) {
    logWarn(
      `Version ${installedVersion} is installed, but the running server is still `
      + `${runningVersion}. Restart NeoAgent to finish the update.`,
    );
  }
...
async function cmdDoctor() {
  ...
  const computerBackend = require('../server/utils/deployment').getTerminalEnv();
  let computerReadiness = null;
  try {
    const { createComputerBackend } = require('../server/services/runtime/backend_factory');
    computerReadiness = createComputerBackend().vmManager.getReadiness();
  } catch {}
  ...
  const checks = [
    ...
    },
    {
      ...
      message: computerReadiness?.ready === true
        ? `${computerBackend} computer runtime is present (${computerReadiness.accelerator || computerBackend})`
        : `${computerBackend} computer runtime is missing or incomplete`,
      action: 'Run `neoagent repair` to restore the computer runtime.',
    },
    {
      id: 'computer-image',
      ok: computerReadiness?.ready === true,
      code: computerReadiness?.ready === true ? null : 'DOCTOR_COMPUTER_IMAGE_UNAVAILABLE',
      message: computerReadiness?.isolated === false
        ? 'No guest image: the runtime is this machine'
        : computerReadiness?.imageReady === true
          ? 'Guest image is present'
          : 'Guest image is prepared on first start',
      action: 'Run `neoagent repair` to prepare the guest image now.',
    },
    {
      id: 'server',
...
async function migrateLegacyPackageInstallation(releaseChannel, versionBefore) {
  heading('Migrate legacy package installation');
  for (const command of ['git', 'npm']) {
    const check = runQuiet(command, ['--version']);
    if (check.error || check.status !== 0) {
      throw new Error(
        `${command} is required to migrate this legacy installation. `
        + 'Install Git and Node.js with npm, then rerun `neoagent update`.',
      );
    }
  }

  const sourceDirectory = defaultSourceInstallDirectory();
  const prepared = prepareSourceInstallation({
    run: (command, args, options = {}) => runQuiet(command, args, options),
    releaseChannel,
    sourceDirectory,
    onInfo: (message) => logInfo(message),
  });
  logOk(`Source checkout prepared on ${prepared.targetBranch}`);

  const targetCli = path.join(sourceDirectory, 'bin', 'neoagent.js');
  const adoption = spawnSync(
    process.execPath,
    [targetCli, 'install', '--adopt-existing'],
    {
      cwd: sourceDirectory,
      env: {
        ...process.env,
  ...
  if (adoption.error || adoption.status !== 0) {
    throw new Error(
      `The Git checkout was prepared at ${sourceDirectory}, but the existing `
      + 'service could not be switched to it. No runtime data was removed; '
      + `rerun: ${process.execPath} ${targetCli} install --adopt-existing`,
    );
  }
  logOk(`Migrated ${versionBefore} -> ${prepared.version}`);
  logOk(`Future updates will use the Git checkout at ${sourceDirectory}`);
}
...
async function cmdUpdate(args = []) {
  ...
  if (fs.existsSync(path.join(APP_DIR, '.git')) && commandExists('git')) {
    ...
    await migrateLegacyPackageInstallation(releaseChannel, versionBefore);
    return;
  }
...
// Jev is TypeSafe's decision model on OpenRouter. The server-wide policy
// lives in NEOAGENT_JEV; `agent` (the default) leaves it to each agent's
// Settings, so it removes the variable instead of writing it.
const JEV_POLICY_LABELS = Object.freeze({
  agent: 'each agent decides in Settings',
  on: 'on for every agent',
...
async function cmdJev(args = []) {
  ...
  if (sub === '--help' || sub === '-h' || sub === 'help') {
    console.log('\nNeoAgent Jev');
    console.log('Usage: neoagent jev [subcommand]');
    console.log('');
    console.log('Jev makes the behind-the-scenes decisions (routing, tool and skill choice,');
    console.log('memory ranking, group-chat turn-taking, verification, browser steps) in a');
    console.log('fraction of a second. Replies are still written by the chat model.');
    console.log('It needs an OpenRouter API key.');
    console.log('');
    console.log('Subcommands:');
    console.log('  neoagent jev         Show the Jev policy and OpenRouter key status');
    console.log('  neoagent jev on      Turn Jev on for every agent and restart');
    console.log('  neoagent jev agent   Let each agent decide in Settings and restart');
    console.log('  neoagent jev off     Turn Jev off on this server and restart');
    console.log('');
    return;
  }

  if (sub === 'status') {
    cliBanner('Jev', 'decision model');
    heading('Jev Status');
    const env = Object.fromEntries(parseEnv(readEnvFileRaw()).entries());
    const configured = String(env.NEOAGENT_JEV || '').trim().toLowerCase();
    const policy = Object.hasOwn(JEV_POLICY_LABELS, configured) ? configured : 'agent';
    const hasKey = Boolean(env.OPENROUTER_API_KEY);
    cliSection('Jev');
    statusLine(policy !== 'off', 'policy', JEV_POLICY_LABELS[policy], policy === 'on' ? '' : 'recommended: neoagent jev on');
    statusLine(hasKey, 'api key', hasKey ? `OpenRouter ${maskEnvValue('KEY', env.OPENROUTER_API_KEY)}` : 'not set', hasKey ? '' : 'run: neoagent env set OPENROUTER_API_KEY <key>');
    console.log('');
    if (!hasKey) {
      logWarn('Jev needs an OpenRouter key: a server key here, or an agent\'s own key in Settings.');
    } else if (policy === 'on') {
      logOk('Jev is on for every agent.');
    } else if (policy === 'agent') {
      logInfo('Each agent turns Jev on under Settings > Models. Run `neoagent jev on` to turn it on for everyone.');
    }
    return;
  }

  if (Object.hasOwn(JEV_POLICY_LABELS, sub)) {
    heading('Jev');
    if (sub === 'agent') {
      removeEnvValue('NEOAGENT_JEV');
      logOk('Removed NEOAGENT_JEV; each agent decides in Settings');
    } else {
      upsertEnvValue('NEOAGENT_JEV', sub);
      logOk(`Set NEOAGENT_JEV=${sub}`);
    }
  ...
  throw new Error(`Unknown jev subcommand: ${sub}. Run "neoagent jev --help" for usage.`);
}
...
async function cmdAccount(args = []) {
  if (args[0] !== 'create') {
    throw new Error('Usage: neoagent account create --username <name> --password <password>');
  }
  const options = {};
  for (let i = 1; i < args.length; i += 1) {
    const key = { '--username': 'username', '--password': 'password' }[args[i]];
    if (!key) throw new Error(`Unknown option: ${args[i]}`);
    const value = args[i + 1];
    if (!value || value.startsWith('--')) {
      throw new Error(`Usage: neoagent account create ${args[i]} <value>`);
    }
    options[key] = value;
    i += 1;
  }
  const { provisionFirstAccount } = require('../server/services/account/provision');
  const account = provisionFirstAccount(options);
  if (cliJsonOutput) {
    process.stdout.write(`${JSON.stringify(account)}\n`);
  } else {
    logOk(`Created account ${account.username}`);
  }
}

// Admin is a flag on a user account, read by the server on every request, so a
// change here applies to a running server immediately.
function cmdAdmin(args = []) {
  const [sub = 'list', username] = args;
  const access = require('../server/services/access/admin');
  if (sub === 'list') {
    const admins = access.listAdmins();
    if (cliJsonOutput) {
      process.stdout.write(`${JSON.stringify({ admins })}\n`);
    } else if (admins.length > 0) {
      for (const admin of admins) logOk(admin.username);
    } else if (access.needsAdminGrant()) {
      logWarn('No account is an admin yet. Run `neoagent admin grant <username>`.');
    } else {
      logInfo('No accounts yet. The first account you create becomes the admin.');
    }
    return;
  }
  if (sub !== 'grant' && sub !== 'revoke') {
    throw new Error('Usage: neoagent admin [list | grant <username> | revoke <username>]');
  }
  if (!username) {
    throw new Error(`Usage: neoagent admin ${sub} <username>`);
  }
  const result = sub === 'grant'
    ? access.grantAdmin(username, { source: 'cli' })
    : access.revokeAdmin(username, { source: 'cli' });
  if (cliJsonOutput) {
    process.stdout.write(`${JSON.stringify(result)}\n`);
  } else if (!result.changed) {
    logInfo(`${result.username} ${sub === 'grant' ? 'is already' : 'is not'} an admin.`);
  } else {
    logOk(`${result.username} ${sub === 'grant' ? 'is now an admin' : 'is no longer an admin'}.`);
  }
}

const EXEC_ACK_FILE = path.join(RUNTIME_HOME, 'exec-acknowledged');

// `exec` runs the agent's shell and file tools directly on this machine, not in
// the cloud VM the server uses. That is the point of the command, but it is
// worth saying out loud once before the first run.
async function confirmLocalExecution(skipPrompt) {
  if (skipPrompt || fs.existsSync(EXEC_ACK_FILE)) return;
  const warning = 'neoagent exec runs commands and edits files directly on this '
    + 'computer, with no virtual machine in between.';
  if (!CLI_INTERACTIVE) {
    throw new Error(`${warning} Re-run with --yes to confirm.`);
  }
  logWarn(warning);
  const answer = await ask('Continue and remember this choice? (y/N)', 'n');
  if (!/^y(es)?$/i.test(answer)) {
    throw new Error('Cancelled.');
  }
  fs.writeFileSync(EXEC_ACK_FILE, `${new Date().toISOString()}\n`);
}

function parseExecArguments(args) {
  const options = {
    instruction: null, model: null, provider: null, agent: null, cwd: null, allowAllTools: false, yes: false,
  };
  const flags = { '--model': 'model', '--provider': 'provider', '--agent': 'agent', '--cwd': 'cwd' };
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (key) {
      const value = args[i + 1];
      if (!value || value.startsWith('--')) {
        throw new Error(`Usage: neoagent exec ${arg} <value> "<instruction>"`);
      }
      options[key] = value;
      i += 1;
      continue;
    ...
    if (arg.startsWith('--')) {
      throw new Error(`Unknown option: ${arg}. Run "neoagent exec --help" for usage.`);
    }
    if (options.instruction !== null) {
      throw new Error('Usage: neoagent exec [options] "<instruction>"');
    }
...
async function cmdExec(args = []) {
  const options = parseExecArguments(args);
  if (!options.instruction) {
    throw new Error('Usage: neoagent exec [options] "<instruction>"');
  }

  await confirmLocalExecution(options.yes);

  // Nobody is present to answer an approval prompt, so a headless run either
  // opts out of the approval gate explicitly or blocks on it. Scoped to this
  // process; the stored security mode is left untouched.
  if (options.allowAllTools) {
    process.env.NEOAGENT_TOOL_SECURITY_MODE = 'allow_all';
  }

  const { runHeadlessTask } = require('../server/services/agents/headless');
  const result = await runHeadlessTask({
    instruction: options.instruction,
    model: options.model,
    provider: options.provider,
    agent: options.agent,
    workspaceRoot: options.cwd ? path.resolve(options.cwd) : process.cwd(),
  });
  ...
  if (result?.status !== 'completed') {
    throw new Error(`Run ${result?.status || 'failed'}${result?.error ? `: ${result.error}` : ''}`);
  }
}
...
function printHelp() {
  ...
  row('install --channel X',  'Bootstrap on the stable or beta channel');
  ...
  cliSection('Run');
  row('account create',      'Create the first account on a fresh install');
  row('exec "<task>"',       'Run one task in the current directory, then exit');
  row('exec --model M --provider P', 'Run one task on a specific provider/model (--model alone also accepts "provider/model")');
  row('exec --allow-all-tools', 'Skip tool approval prompts for that run');
  row('exec --yes',          'Skip the local-execution warning (non-interactive)');
  console.log('');
  ...
  row('update',              'Replace source checkout with latest on current channel');
  row('update stable|beta',  'Replace source checkout and switch channel');
  ...
  cliSection('Admin');
  row('admin',               'List admin accounts');
  row('admin grant <user>',  'Make an account an admin');
  row('admin revoke <user>', 'Remove admin from an account');
  console.log('');

  cliSection('Jev');
  row('jev',                 'Show Jev decision model status');
  row('jev on',              'Turn Jev on for every agent and restart');
  row('jev agent',           'Let each agent decide in Settings and restart');
  row('jev off',             'Turn Jev off on this server and restart');
  console.log('');
...
async function runCLI(argv) {
  ...
  switch (command) {
    ...
    case 'billing':
      ...
      break;
    case 'jev':
      await cmdJev(argv.slice(1));
      break;
    case 'admin':
      cmdAdmin(argv.slice(1));
      break;
    case 'account':
      await cmdAccount(argv.slice(1));
      break;
    case 'exec':
