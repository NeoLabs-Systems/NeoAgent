...
function defaultEnvLines(current = {}) {
  ...
  return [
    ...
    `NEOAGENT_SETUP_CLAIM_REQUIRED=${current.NEOAGENT_SETUP_CLAIM_REQUIRED || 'true'}`,
...
function createFirstUserClaim() {
  const {
    createSetupClaim,
    getSetupHandshake,
  } = require('../server/services/setup/onboarding');
  if (getSetupHandshake().claimed) return null;
  return createSetupClaim();
}

function printSetupClaim(claim, backendUrl) {
  if (!claim) {
    logInfo(`Open ${backendUrl} to sign in.`);
    return;
  }
  logInfo(`Open ${backendUrl} and enter the one-time setup code: ${claim.token}`);
  logWarn('The setup code expires in 15 minutes and can only be used once.');
}

function cmdClaim() {
  const backendUrl = `http://localhost:${loadEnvPort()}`;
  printSetupClaim(createFirstUserClaim(), backendUrl);
}
...
async function cmdInstall({ args = [], showBanner = true } = {}) {
  ...
  try {
    ...
    } else if (options.runtimePackage) {
    ...
    if (profile === 'full' && !options.nonInteractive && !options.json) {
      const prepareIsolation = await ask(
        'Prepare the optional isolated browser/CLI runtime now? (y/N)',
        'N',
      );
      if (['y', 'yes'].includes(prepareIsolation.trim().toLowerCase())) {
        await ensureContainerRuntime({ required: false });
      } else {
        rememberInstallAction('Prepare browser isolation later with `neoagent setup --full`.');
      }
      ...
      rememberInstallAction('Optional browser isolation, integrations, voice, and media tools can be added later.');
    }
    ...
    const claim = createFirstUserClaim();
    ...
    if (!cliJsonOutput) {
      ...
      heading('Ready');
      printSetupClaim(claim, backendUrl);
...
// Lifecycle commands manage a per-user service, a checkout, and log files that
// the Docker image does not own. Inside the container the equivalent action
// belongs to Compose on the host.
const COMPOSE_COMMANDS = {
  install: 'docker compose up -d',
  start: 'docker compose start',
  stop: 'docker compose stop',
  restart: 'docker compose restart neoagent',
  update: 'docker compose build --pull && docker compose up -d',
  repair: 'docker compose up -d --build --force-recreate',
  fix: 'docker compose up -d --build --force-recreate',
  uninstall: 'docker compose down',
  logs: 'docker compose logs -f neoagent',
};

function runsInContainer() {
  return String(process.env.NEOAGENT_CONTAINER || '').trim() === '1';
}

function assertHostCommand(command) {
  const composeCommand = COMPOSE_COMMANDS[command];
  if (!composeCommand || !runsInContainer()) return;
  throw new Error(
    `\`neoagent ${command}\` is managed by Docker in this installation. Run \`${composeCommand}\` on the host instead.`,
  );
}

function cmdRestart() {
  heading(`Restart ${APP_NAME}`);
  // Commands that change credentials or billing restart the server themselves.
  // In a container the server is the container, so the operator applies it.
  if (runsInContainer()) {
    logInfo(`Run \`${COMPOSE_COMMANDS.restart}\` on the host to apply the change.`);
    return;
  }
...
function printHelp() {
  ...
  row('claim',               'Print a one-time first-account setup code');
  console.log('');
