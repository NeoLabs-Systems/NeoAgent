...
async function ensureComputerRuntime({ required = false, downloadImage = true } = {}) {
  ...
  try {
    ...
    let vmManager = new QemuVMManager();
    if (!vmManager.getReadiness().qemuAvailable) {
      logInfo('QEMU computer runtime is missing. Restoring…');
