...
async function cmdStatus({ showBanner = true } = {}) {
  ...
  try {
    ...
    const computer = createComputerVmManager().getReadiness();
    statusLine(
      computer.ready,
      'computer',
      computer.ready
        ? `${computer.accelerator || backend} · ${computer.imageReady ? 'guest image ready' : 'guest image is prepared on first start'}`
...
async function cmdDoctor() {
  ...
  try {
    const { createComputerVmManager } = require('../server/services/runtime/vm_manager');
    computerReadiness = createComputerVmManager().getReadiness();
  } catch {}
