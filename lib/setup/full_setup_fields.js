...
const { LIVE_VOICE_PROVIDERS } = require('../../server/services/voice/live/catalog');
...
// Live calls use a speech-to-speech model with the provider's AI key; accounts
// can pick a different one in the app. Deepgram only transcribes voice notes.
const VOICE_FIELDS = Object.freeze([
  {
    key: 'VOICE_LIVE_PROVIDER',
    label: `Live voice provider (${Object.keys(LIVE_VOICE_PROVIDERS).join(' or ')})`,
    defaultValue: 'openai',
  },
  { key: 'VOICE_LIVE_MODEL', label: 'Live voice model (blank for the provider default)' },
  { key: 'VOICE_LIVE_VOICE', label: 'Live voice (blank for the provider default)' },
  { key: 'DEEPGRAM_API_KEY', label: 'Deepgram API key for voice-note transcription (optional)', secret: true },
]);
...
const SECTION_COMPLETION_KEYS = Object.freeze({
  ...
  voice: Object.freeze(['VOICE_LIVE_PROVIDER']),
});
