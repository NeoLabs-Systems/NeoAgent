...
const SETUP_SCHEMA_VERSION = SETUP_CONTRACT.schemaVersion;
...
function parseSetupArguments(args = []) {
  ...
  const channelIndex = values.findIndex(
    (value) => value === '--channel' || value.startsWith('--channel='),
  );
  let releaseChannel = null;
  if (channelIndex >= 0) {
    const flag = values[channelIndex];
    releaseChannel = flag.includes('=')
      ? flag.slice(flag.indexOf('=') + 1).trim()
      : String(values[channelIndex + 1] || '').trim();
  }
  return {
