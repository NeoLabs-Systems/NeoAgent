...
function configuredPublicKey() {
  ...
  if (environmentKey) return normalizeRuntimeSigningPublicKey(environmentKey);
  const embeddedKey = typeof RUNTIME_SIGNING_PUBLIC_KEY === 'undefined'
    ? ''
    : String(RUNTIME_SIGNING_PUBLIC_KEY).trim();
  if (embeddedKey) return normalizeRuntimeSigningPublicKey(embeddedKey);
  return canonicalRuntimeSigningPublicKey();
}
...
function validateDownloadUrl(value) {
  ...
  if (!url || url.protocol !== 'https:') {
    const error = new Error('NeoAgent runtime downloads require HTTPS.');
    error.code = 'SETUP_DOWNLOAD_URL_INVALID';
    throw error;
  }
  return url;
}
...
async function openDownload(url) {
  ...
  if (!response.body) {
    const error = new Error('The runtime download returned an empty response.');
    error.code = 'SETUP_DOWNLOAD_FAILED';
    throw error;
  }
  return response;
...
async function fetchBytes(url, { maxBytes = 2 * 1024 * 1024 } = {}) {
  ...
  for await (const chunk of response.body) {
    size += chunk.length;
    if (size > maxBytes) {
      const error = new Error('NeoAgent setup metadata exceeded its safe size limit.');
      error.code = 'SETUP_DOWNLOAD_TOO_LARGE';
      throw error;
    }
  ...
  try {
    for await (const chunk of response.body) {
      ...
      if (received > expectedSize) {
        const error = new Error('The NeoAgent runtime download exceeded its manifest size.');
    ...
    fs.closeSync(file);
    if (!verified) fs.rmSync(target, { force: true });
  }
}
...
function selectArtifact(manifest) {
  ...
  try {
    return normalizeArtifactMetadata(artifact);
  } catch (error) {
    error.code = 'SETUP_MANIFEST_INVALID';
    throw error;
  }
...
async function installRuntime(options = {}) {
  ...
  const signature = (await fetchBytes(
  ...
  let version;
  try {
    version = normalizeRuntimeVersion(manifest.version);
  } catch (error) {
    error.code = 'SETUP_MANIFEST_INVALID';
    throw error;
  }
  ...
  try {
    await downloadRuntimeArchive(assetUrl, archivePath, {
