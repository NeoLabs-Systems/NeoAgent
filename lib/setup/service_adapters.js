...
function renderSystemdUnit({
  ...
  if (runtimeHome) {
    lines.push(`Environment=NEOAGENT_HOME=${quoteSystemdValue(runtimeHome)}`);
  }
  lines.push(
    `StandardOutput=append:${path.join(logDir, 'neoagent.log')}`,
    `StandardError=append:${path.join(logDir, 'neoagent.error.log')}`,
    '',
    '[Install]',
