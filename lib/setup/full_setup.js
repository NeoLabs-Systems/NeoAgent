...
function normalizeLiveVoiceProvider(value) {
  const normalized = String(value || '').trim().toLowerCase();
  if (!normalized || LIVE_VOICE_PROVIDERS[normalized]) return normalized || undefined;
  const error = new Error(
    `Live voice provider must be one of: ${Object.keys(LIVE_VOICE_PROVIDERS).join(', ')}.`,
  );
  error.code = 'SETUP_VALUE_INVALID';
  throw error;
}
...
function buildEnvironmentUpdates(draft) {
  ...
  return {
    ...
    VOICE_LIVE_PROVIDER: normalizeLiveVoiceProvider(draft.VOICE_LIVE_PROVIDER),
  };
}
