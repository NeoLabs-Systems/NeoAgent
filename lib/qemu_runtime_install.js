...
const { ensureMacQemuHypervisorSignature } = require('./qemu_mac_sign');
...
function ensureStagedMacQemuSignature(directory, architecture) {
  const binary = path.join(directory, 'bin', executableName(systemBinaryName(architecture)));
  return ensureMacQemuHypervisorSignature(binary);
}
...
function restoreQemuComputerRuntime(options = {}) {
  ...
  const allowHostInstall = options.allowHostInstall !== false;
  if (isCompleteQemuRuntime(destination, architecture)) {
    if (ensureStagedMacQemuSignature(destination, architecture)) {
      return { action: 'signed', directory: destination };
    }
  ...
  if (isCompleteQemuRuntime(packaged, architecture)) {
    ...
    ensureStagedMacQemuSignature(destination, architecture);
    return { action: 'copied', directory: destination, source: packaged };
  }

  if (!allowHostInstall) {
    return { action: 'missing', directory: destination };
  }
