...
async function cmdApiKeySetup() {
  ...
  logInfo('Choose one or more hosted providers. You can also configure these later from the admin dashboard or with `neoagent env set`.');
  ...
  if (choices.length === 0) {
    logInfo('Skipping hosted provider keys. Add them from the admin dashboard or with `neoagent env set`. Local Ollama remains available if OLLAMA_URL is reachable.');
    rememberInstallAction('Add a hosted provider later from the admin dashboard or with `neoagent env set`.');
    return 0;
  }
...
function writeDefaultEnvFile({ remindProviderSetup = true, port = null } = {}) {
  ...
  if (remindProviderSetup) {
    rememberInstallAction('Add provider keys with `neoagent setup`, `neoagent env set KEY VALUE`, the admin dashboard, or the login commands when you are ready.');
  }
}
  ...
  if (normalizedProfile === 'quick') {
    ...
    if (!options.nonInteractive && process.stdin.isTTY && process.stdout.isTTY) {
      ...
      if (!['n', 'no'].includes(configureProvider.trim().toLowerCase())) {
        ...
        rememberInstallAction('Connect an AI provider later from the admin dashboard or with `neoagent env set`.');
      }
    } else {
      rememberInstallAction('Connect an AI provider later from the admin dashboard or with `neoagent env set`.');
    }
    ...
    rememberInstallAction(
      'Configure AI providers from the admin dashboard or `neoagent env`. Continue integrations, voice, and optional tools in the NeoAgent app.',
    );
    completedSections = ['core'];
  } else {
...
function parseExecArguments(args) {
  const options = {
    instruction: null, model: null, agent: null, cwd: null, allowAllTools: false, yes: false,
  };
  const flags = { '--model': 'model', '--agent': 'agent', '--cwd': 'cwd' };
...
function printHelp() {
  ...
  row('exec --model P/M',    'Run one task on a specific provider/model');
